...
function CreateGameContent() {
  ...
  async function handleCreate() {
    ...
    if (sessionErr || !session) {
      setError('ゲームセッションの作成に失敗しました: ' + (sessionErr?.message ?? ''));
      setCreating(false);
      return;
    }
      ...
      <div className="mx-auto max-w-lg space-y-6 text-center">
        <div className="rounded-lg border border-green-200 bg-green-50 p-8 dark:border-green-900 dark:bg-green-950">
          ...
          <div className="mt-4 rounded-lg bg-white p-6 dark:bg-gray-900">
            ...
            <p className="mt-1 font-mono text-4xl font-bold tracking-[0.3em] text-indigo-600">
              {created.game_code}
            </p>
          </div>
          <p className="mt-4 text-sm text-gray-600 dark:text-gray-400">
            このコードを生徒に伝えるか、ホスト画面のQRコードを見せてください
          </p>
        </div>

        <div className="flex flex-col gap-3">
          <a
            href={`/host/${created.game_code}`}
          ...
          <button
            onClick={() => {
              navigator.clipboard.writeText(created.game_code);
            }}
            className="rounded-lg border border-gray-300 px-6 py-3 text-sm font-medium hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800"
          >
            📋 コードをコピー
          </button>
          <button
          ...
          <button
            onClick={() => {
              setCreated(null);
              setSelectedId(preselectedId ?? gameSets[0]?.id ?? '');
            }}
    ...
    <div className="mx-auto max-w-lg space-y-6">
      ...
      {gameSets.length === 0 ? (
        <div className="rounded-lg border border-dashed border-gray-300 p-8 text-center dark:border-gray-700">
          ...
          <a href="/admin/import" className="mt-2 inline-block text-sm text-blue-600 hover:underline">
            まずはExcelインポートしてください →
          </a>
        </div>
      ) : (
            ...
            >
              {gameSets.map((gs) => (
                <option key={gs.id} value={gs.id}>
                  {gs.name} （{gs.dice_count}d{gs.dice_sides}）
                </option>
              ))}
            </select>
          </div>
